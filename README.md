...
## SOME ADVICES
  ...
  - ENCRYPTED HARD DRIVES: If you have hard drives encrypted using the openmediavault-luksencryption omv-regen plugin will need to decrypt them to be able to do the regeneration without errors. To do this you must create a /etc/crypttab file following the Debian manual and establish the paths to the files with the decryption keys. omv-regen will transfer these files to the backup and then to the new system. During the regeneration, a reboot will be necessary to decrypt those hard drives, omv-regen will notify you, after the reboot you must run omv-regen again to continue the process.
